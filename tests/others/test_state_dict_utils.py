...
class StateDictUtilsTest(unittest.TestCase):
    ...
    def test_convert_state_dict_to_kohya_from_peft(self):
        down_weight = torch.ones(2, 4)
        state_dict = {
            "unet.down_blocks.0.attentions.0.to_q.lora_A.weight": down_weight,
            "unet.down_blocks.0.attentions.0.to_q.lora_B.weight": torch.ones(4, 8),
        }
        converted = convert_state_dict_to_kohya(state_dict)
        self.assertTrue(any("lora_unet" in key for key in converted))
        self.assertTrue(any("lora_down" in key for key in converted))
        alpha_keys = [key for key in converted if key.endswith(".alpha")]
        self.assertEqual(len(alpha_keys), 1)
        self.assertEqual(converted[alpha_keys[0]].item(), len(down_weight))
