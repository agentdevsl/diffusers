...
@dataclass
# Copied from diffusers.schedulers.scheduling_ddpm.DDPMSchedulerOutput with DDPM->DDIMVariant
class DDIMVariantSchedulerOutput(BaseOutput):
    """
...
class DDIMVariantScheduler(SchedulerMixin, ConfigMixin):
    """
    `DDIMVariantScheduler` extends the denoising procedure introduced in denoising diffusion probabilistic models (DDPMs) with
    ...
    def step(
    ...
    ) -> DDIMVariantSchedulerOutput | tuple:
        """
        ...
        Args:
            ...
            return_dict (`bool`, *optional*, defaults to `True`):
                Whether or not to return a [`~schedulers.scheduling_ddim_variant.DDIMVariantSchedulerOutput`] or `tuple`.

        Returns:
            [`~schedulers.scheduling_ddim_variant.DDIMVariantSchedulerOutput`] or `tuple`:
                If return_dict is `True`, [`~schedulers.scheduling_ddim_variant.DDIMVariantSchedulerOutput`] is returned, otherwise a
        ...
        return DDIMVariantSchedulerOutput(prev_sample=prev_sample, pred_original_sample=pred_original_sample)
